...
class TasksInterfaceTest < TasksInterface
  ...
  test "should show errors but not create task on invalid submission" do
    assert_no_difference 'Task.count' do
      post tasks_path, params: { task: { title: "", state: false, due_date: "2023-02-09", priority: 5, memo: "なるはや" } }
    end
  ...
  test "should create a task on valid submission" do
    assert_difference 'Task.count', 1 do
      post tasks_path, params: { task: { title: "task1", state: false, due_date: "2023-02-09", priority: 5, memo: "なるはや" } }
    end
